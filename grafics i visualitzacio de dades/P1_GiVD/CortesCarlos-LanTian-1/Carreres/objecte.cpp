...
Capsa3D Objecte::calculCapsa3D(){
    ...
    //Iterar els punts (vertexs) de l'objecte per obtenir el punt minim i maxim, els quals poden definir una CMC.
    for (int i=0; i<vertexs.size();i++){
        point4 vertex = vertexs[i];
        if (pmin.x>vertex.x)  pmin.x=vertex.x;
        if (pmin.y>vertex.y)  pmin.y=vertex.y;
        if (pmin.z>vertex.z)  pmin.z=vertex.z;
        if (pmax.x<vertex.x)  pmax.x=vertex.x;
        if (pmax.y<vertex.y)  pmax.y=vertex.y;
        if (pmax.z<vertex.z)  pmax.z=vertex.z;
    }
